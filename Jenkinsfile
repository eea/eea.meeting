pipeline {
  ...
  environment {
        ...
        SONARQUBE_TAGS = "land.copernicus.eu,clmsdemo.devel6cph.eea.europa.eu"
    }
  stages {
        ...
        node(label: 'swarm') {
          script{
            ...
            withSonarQubeEnv('Sonarqube') {
                sh "export PATH=$PATH:${scannerHome}/bin:${nodeJS}/bin; sonar-scanner -Dsonar.sources=./src/eea  -Dsonar.projectKey=\"${GIT_NAME}\" -Dsonar.projectName=\"${GIT_NAME}\" -Dsonar.branch.name=\"${BRANCH_NAME}\""
